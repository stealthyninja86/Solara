    ...
    <aside className="flex h-full w-[220px] shrink-0 flex-col bg-[var(--color-surface)]">
      <div className="flex items-center justify-end px-4 pb-1 pt-4">
        <button
          ...
          className="ml-auto! mt-0! w-auto! cursor-pointer rounded-md bg-transparent! p-1! text-[0.75rem]! leading-none text-[var(--color-text-muted)]! transition-colors hover:bg-[var(--color-bg-hover)]! hover:text-[var(--color-text)]!"
        >
          «
        </button>
      </div>
      ...
      <div className="px-4 pb-5">
        <button
          ...
          className="mx-1 mt-0! flex w-[calc(100%-0.5rem)] cursor-pointer items-center gap-2.5 rounded-md bg-transparent! px-3! py-2! text-caption! text-[var(--color-text-muted)]! transition-colors hover:bg-[var(--color-bg-hover)]! hover:text-[var(--color-danger)]!"
        >
          <Icon name="logout" size={14} />
          Logout
        </button>
        <div className="px-3 pt-1.5">
          <div className="truncate text-tiny">{auth.email}</div>
        </div>
      </div>
    </aside>
  );
