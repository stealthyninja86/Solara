  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="mt-0! flex w-auto! cursor-pointer items-center gap-1.5 rounded-md bg-transparent! px-2! py-1! text-[0.8rem]! text-[var(--color-text-muted)]! transition-colors hover:bg-[var(--color-bg-hover)]! hover:text-[var(--color-text-secondary)]!"
      >
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-1 w-40 overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] shadow-xl">
          <div className="px-3 py-2 text-[0.65rem] font-medium uppercase tracking-wider text-[var(--color-text-muted)]">
            Theme
          </div>
          {OPTIONS.map((option) => (
            <button
              ...
              className={`mt-0! flex w-auto! cursor-pointer items-center gap-2.5 bg-transparent! px-3! py-2! text-[0.8rem]! transition-colors hover:bg-[var(--color-bg-hover)]! ${
                theme === option.value
                  ? "text-[var(--color-text)]!"
